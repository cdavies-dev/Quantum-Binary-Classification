...
class QuantumNeuralNetwork():
    def __init__(self, x_train, y_train, x_test, y_test, epochs, layers):
        ...
        model.save('QML Amplitude Encoding (0-1) + Ansatz Variation 1, 2 layers + Adjoint Differentiation.h5')
        history_df = pd.DataFrame(history.history)
        with open('QML Amplitude Encoding (0-1) + Ansatz Variation 1, 2 layers + Adjoint Differentiation.csv', mode = 'w') as f:
            history_df.to_csv(f)
        print('\nModel complete!\n')
