...
class QuantumNeuralNetwork():
    def __init__(self, x_train, y_train, x_test, y_test, epochs, layers):
        ...
        model.save('QML Amplitude Encoding (0-1) + Ansatz Variation 2, 2 layers.h5')
        history_df = pd.DataFrame(history.history)
        with open('QML Amplitude Encoding (0-1) + Ansatz Variation 2.csv, 2 layers', mode = 'w') as f:
            history_df.to_csv(f)
        print('\nModel complete!\n')
    ...
    def generate_model(self, x_train, layers):
        ...
        def qnode(inputs, weights):
            qml.AmplitudeEmbedding(inputs, wires = range(10), pad_with = 0.0, normalize = True, do_queue = True, id = None) #pad_width changed from 240
            ...
            for jj in range(n_layers):
                ...
                for ii in range(floor(n_qubits / 2)):
                    qml.CNOT(wires=[2 * ii + 1, 2 * ii])

                for ii in range(n_qubits - 1):
                    qml.RZ(weights[jj, n_qubits + ii, 0], wires = (ii))
                    qml.RY(weights[jj, n_qubits + ii, 1], wires = (ii))
                    qml.RZ(weights[jj, n_qubits + ii, 2], wires = (ii))
                    qml.RZ(weights[jj, 2 * n_qubits + ii - 1, 0], wires = ((ii + 2) % n_qubits))
                    qml.RY(weights[jj, 2 * n_qubits + ii - 1, 1], wires = ((ii + 2) % n_qubits))
                    qml.RZ(weights[jj, 2 * n_qubits + ii - 1, 2], wires = ((ii + 2) % n_qubits))
                    qml.CNOT(wires = [((ii + 2) % n_qubits), ii])

                qml.RZ(weights[jj, 3 * (n_qubits) - 2, 0], wires = 0)
                qml.RY(weights[jj, 3 * (n_qubits) - 2, 1], wires = 0)
                qml.RZ(weights[jj, 3 * (n_qubits) - 2, 2], wires = 0)

            return qml.expval(qml.PauliZ(0))

        weight_shapes = {'weights': (n_layers, 3 * n_qubits - 1, 3)}
