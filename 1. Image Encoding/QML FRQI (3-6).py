...
class QuantumNeuralNetwork():
    def __init__(self, x_train, y_train, x_test, y_test, epochs, layers):
        ...
        model.save('QML FRQI (3-6).h5')
        history_df = pd.DataFrame(history.history)
        with open('QML FRQI (3-6).csv', mode = 'w') as f:
            history_df.to_csv(f)
        print('\nModel complete!\n')
    ...
    def generate_model(self, x_train, layers):
        ...
        def qnode(inputs, weights):
            inputs = frqi(inputs)
            qml.AmplitudeEmbedding(inputs, wires = range(n_qubits), pad_with = 0, normalize = True, do_queue = True, id = None)
        ...
        def frqi(inputarray):
            ...
            for myinput in inputarray:
                frqiarray.extend([(1 / sqrt(2)) * np.cos(myinput), (1 / sqrt(2)) * np.sin(myinput)])
